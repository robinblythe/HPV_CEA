...
p_cancer <- df_cancer |>
  rename(Sex = Gender) |>
  filter(Diagnosis %in% c("All cancers", "Oropharyngeal")) |>
  mutate(Diagnosis = ifelse(Diagnosis == "Oropharyngeal", "Oropharyngeal only", "All HPV cancers")) |>
  ggplot(aes(x = NMB_vc, y = Diagnosis, fill = Diagnosis))

p_cancer_summary <- df_cancer_results |>
  rename(Sex = Gender) |>
  filter(Diagnosis %in% c("All cancers", "Oropharyngeal")) |>
  mutate(Diagnosis = ifelse(Diagnosis == "Oropharyngeal", "Oropharyngeal only", "All HPV cancers"))
...
p_cancer +
  ...
  scale_fill_manual(values = c("#EF7C00", "#003D7C"), guide = "none") +
  theme_bw() +
  facet_wrap2(vars(Sex), scales = "free", ncol = 1) +
  geom_label(data = p_cancer_summary, 
             aes(y = Diagnosis, x = NMB_vc_median, label = scales::dollar(round(NMB_vc_median), prefix = "SGD")), 
             position = position_nudge(y = -0.2),
             colour = "white") +
  theme(panel.grid.minor = element_blank()) +
  xlab("Net economic value of HPV vaccination per individual (2025)") +
  facetted_pos_scales(x = list(
    scale_x_continuous(limits = c(-200, 2000), breaks = seq(-200, 2000, 200)),
    scale_x_continuous(limits = c(-150, 0, 50)),
    y = NULL))

ggsave(filename = "./Oropharyngeal_only/Figure density.jpg", height = 8, width = 12)
...
p <- summary |>
  ...
  filter(Diagnosis == "Oropharyngeal") |>
  ggplot(aes(
    x = Diagnosis_age,
...
p +
  ...
  facet_wrap(vars(Gender), nrow = 1, ncol = 2, axes = "all_x") +
  theme_bw() +
  theme(panel.grid.minor = element_blank()) +
  scale_y_continuous(
...
ggsave(filename = "./Oropharyngeal_only/lifetime_income_loss.jpg", height = 6, width = 10)
