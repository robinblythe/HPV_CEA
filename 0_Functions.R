...
run_sensitivity <- function(cancer_type, gender, seed, iteration) {
  ...
  diag_prob <- cancer_rate_base$diag_prob[idx]
  diag_prob_herd <- cancer_rate_base$diag_prob[idx] * herd_immunity
  ...
  sim <- tibble(
    ...
    EV_cancer_herd_no_vc = 
      Lost_income_cancer * diag_prob * (1 - pct_hpv) +
      Lost_income_cancer * diag_prob_herd * pct_hpv,
    ...
    Vaccination_benefit_herd_no_vc = EV_cancer_herd_no_vc - EV_cancer_no_vc
  )
}



run_sensitivity_deterministic <- function(cancer_type, gender, seed, iteration) {
